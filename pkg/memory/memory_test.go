...
var _ = Describe("Scenario", func() {
	Describe("Name", func() {
		It("returns 'memory'", func() {
			s := memory.New(memory.Config{MaxUseBytes: 10 * 1024 * 1024})
	...
	Describe("Run", func() {
		...
		It("survives more runs than pool capacity (eviction path)", func() {
			...
			s := memory.New(memory.Config{MaxUseBytes: 2 * 1024 * 1024})
			for range 10 {
				result := s.Run(context.Background())
				Expect(result.Err).NotTo(HaveOccurred())
			}
		...
		It("works with the minimum pool size", func() {
			// MaxUseBytes < 1 MB block → clamped to 1 block.
			s := memory.New(memory.Config{MaxUseBytes: 0})
			result := s.Run(context.Background())
			Expect(result.Err).NotTo(HaveOccurred())
		})
	})
